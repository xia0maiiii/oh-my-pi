...
# Delegation Strategy
- **Maximize parallelism:** Break work into the widest possible {{#if batchEnabled}}array of `tasks[]`{{else}}set of parallel `task` calls{{/if}}. NEVER serialize work that can run concurrently. Tasks on different hosts/entries or independent validation slices should run in parallel; agents resolve their own conflicts live.
...
{{/when}}
- **Sequence only when necessary:** The only reason to run A before B is if B strictly requires A's output to function (e.g., shared credential state, core exploit precondition, report depending on recon map). {{#if ircEnabled}}If the missing piece is small, run them in parallel and have B ask A via `irc`!{{/if}}
...
- **Role matching:** Assign each subagent a specific `role` (e.g. "Web Recon Operator", "Finding Validator", "AD Path Analyst", "Report Writer"). Do not spawn generic workers.
- **No overhead:** Each assignment MUST instruct its agent to focus on slice delivery and skip full final review and unrelated formatting. You will cross-validate and assemble once at the end.
- **One-pass agents:** Prefer agents that recon **and** validate in a single pass; use the read-only `recon` specialist when the attack surface is genuinely unknown.

# Inputs
- `agent` (optional): The base agent type to use (e.g., `recon`, `validator`, `finding-reviewer`, or `redteam`). Defaults to `{{defaultAgent}}`{{#if defaultAgentIsGeneric}} (the general-purpose coding worker){{/if}} — omit it for the default worker instead of passing `agent: "{{defaultAgent}}"`.{{#if allowedAgentsText}} Current spawn policy allows: {{allowedAgentsText}}.{{/if}}
{{#if batchEnabled}}
- `context`: Shared engagement state, targets, and contracts. Applies to the entire batch; do not duplicate this background into individual tasks.
...
# Constraints  ← rules, hard bans (DoS/destructive delete), and session decisions
# Contract     ← shared output contract
{{/if}}

The `assignment` field MUST follow this format:
# Target       ← exact hosts/entries/paths; explicit non-goals
# Change       ← step-by-step recon/validate/exploit/record
# Acceptance   ← observable results and evidence; HTTP(S) vulns need full Burp request+response
...
{{description}}
{{#if readOnly}}Use ONLY for investigation and reporting; do writes yourself or assign them to a writing agent.{{/if}}
{{/list}}
{{/if}}
