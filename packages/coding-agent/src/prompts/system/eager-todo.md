...
You MUST cover the entire request from modeling, investigation/execution through verification and convergence — not just the next immediate step.
...
{{else}}
Consider calling `{{toolRefs.todo}}` first to lay out a phased plan with a single `init` op. A good list covers the whole request — modeling, investigation/execution through verification and convergence — not just the next step, with specific task descriptions a future turn could execute without re-planning.
