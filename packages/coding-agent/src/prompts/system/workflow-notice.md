<system-notice>
The user's message above contains the **workflowz** keyword: drive this task as a deterministic multi-subagent workflow. Author the orchestration in the `eval` tool and fan out subagents — to ensure attack-surface coverage (decompose independent observation surfaces), to strengthen confidence in conclusions (independent perspectives and adversarial falsification before adjudication), or to take on scale one context can't hold (large audits, cross-system attack chains, broad code/artifact analysis). This overrides any default tendency to do the whole task inline when fanning out would be more thorough.

<when>
Worth it when the task benefits from decomposition + parallel coverage, or from independent/adversarial cross-checking before reaching a conclusion. For a quick lookup, single fact check, or tiny edit, just do it directly — don't spin up agents. Scout inline FIRST (identify the targets, list the key anchors, scope the attack surface and evidence gaps) to discover the work-list, then fan out over it — you don't need to know its full shape before the *task*, only establish sufficient slicing boundaries before the *fan-out*. Common shapes, each a well-scoped `eval` call you can chain across turns:
- **Understand** — parallel readers over subsystems/protocol layers → structured attack-surface map
- **Model** — judge panel of N independent models → falsify, then synthesize the primary attack path
- **Review** — split by entry point, identity, state, boundary, or sink → find → adversarially verify each finding
- **Research** — multi-source sweep → deep-read version and implementation hits → synthesize
- **Verify** — discover candidate paths → independently reproduce with negative controls → confirm the combined chain
</when>
...
**Python (`eval`, Python backend):**
    ...
    DIMENSIONS = [{"key": "entrypoints", "prompt": "…"}, {"key": "state", "prompt": "…"}]
...
**JavaScript (`eval`, JavaScript backend):**
    ...
    const DIMENSIONS = [{ key: "entrypoints", prompt: "…" }, { key: "state", prompt: "…" }];
...
- **Perspective-diverse verify** — give each verifier a distinct lens (reachability, state preconditions, impact, alternative explanations, does-it-reproduce) instead of N identical refuters.
...
- **Budget/count loops** — Python: `while len(findings) < 10:`; JavaScript: `while (findings.length < 10) { … }`. In Python, gate an explicit budget with `budget.total` and `budget.remaining()`; in JavaScript, use `await budget.total()` and `await budget.remaining()`. `log()` each round.
...
Scale to the ask: "find any real attack path" → a few finders, single-vote falsification. "thoroughly audit / ensure coverage" → larger finder pool, 3–5-vote adversarial pass, a completeness-critic stage, and final attack-chain synthesis.
</patterns>
...
- After a fan-out returns, YOU own the truth of the conclusions: read the artifacts, check the sources, run decisive scenarios and negative controls before acting. Subagents do the legwork; they don't get the last word on findings or attack chains.
