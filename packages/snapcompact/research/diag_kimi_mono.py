...
def fw_complete(messages: list[dict], max_tokens: int = 32768) -> dict:
    ...
    out = _post(FW_URL, body, {"authorization": f"Bearer {key}", "user-agent": "diag/1.0"})
...
def main() -> None:
    ...
    for batch, messages in batches:
        qa = cached(
            "fireworks/kimi-k2p6", "qa-mono-prod", {"messages": messages, "effort": None},
    ...
    rows = [
        {
            "model": "fireworks/kimi-k2p6", "cond": cond, "pos_rel": q["pos_rel"], "q": q["q"], "answer": a,
    ...
    summary = {
        "cond": cond, "n": len(rows), "imgs": len(pngs),
    ...
    print(f"{cond} imgs={summary['imgs']} f1={summary['f1']:.3f} em={summary['em']:.3f} "
